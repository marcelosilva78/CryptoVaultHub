...
export const integrationsArticles: Article[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: "paragraph",
        text: "A coleção Postman foi disparada contra a API real (BSC mainnet, projeto BrPay, com transações confirmadas on-chain). Cada request da coleção corresponde a um passo da suíte de integração; juntos eles reproduzem o ciclo completo: smoke test de auth, cadastro de webhook, geração de deposit address determinístico (CREATE2), confirmação do sweep, whitelist com 2FA, saque com idempotency key, e os endpoints de leitura para painéis (listagem enriquecida de forwarders, deposits filtrados por janela de data inclusiva, flush activity feed).",
      },
      {
        type: "callout",
        variant: "tip",
      ...
      },
      {
        ...
        rows: [
          ...
          ["apiKey", "SIM", "cvh_live_REPLACE_ME", "Portal → Settings → API Keys → Create. Escopos mínimos: wallets:read, deposits:read, withdrawals:read, gas-tanks:read, forwarders:read. Para o fluxo E2E completo, inclua forwarders:create, address-book:write, webhooks:write, withdrawals:hot."],
          ...
          ["totp2faCode", "SIM (antes do passo 9)", "(vazio)", "Código TOTP de 6 dígitos do app autenticador. Sem ele, POST /addresses retorna 401."],
          ["portalJwt", "SIM (apenas para api-keys)", "(vazio)", "JWT da sessão do portal (cookie cvh_session). Os endpoints de api-keys são JWT-only — rejeitam X-API-Key."],
          ["projectId", "Não — auto", "(vazio)", "Preenchida em Setup #1 a partir da resposta de GET /projects"],
          ["depositAddress", "Não — auto", "(vazio)", "Preenchida em Deposit #1 (endereço hex 0x...)"],
          ["depositAddressId", "Não — auto", "(vazio)", "Preenchida em Deposit #1 (numeric id) — usada por POST /deposit-addresses/:id/balances"],
          ["webhookId", "Não — auto", "(vazio)", "Preenchida em Webhook #1 quando você cadastra o webhook"],
          ["withdrawalId", "Não — auto", "(vazio)", "Preenchida em Withdrawal #1 quando o saque é criado"],
          ["keyId", "Não — auto", "(vazio)", "Preenchida pela request List/Create API key — usada por DELETE /api-keys/:id"],
          ["rangeFrom / rangeTo", "Não — auto", "(vazio)", "Calculadas no pre-request de 'Deposits filtered' como hoje-7 / hoje (UTC)"],
      ...
      },
      {
        ...
        ordered: true,
        items: [
          "**1. Setup** — Resolve o projeto, lista wallets, valida saúde do gas tank via GET /gas-tanks (a listagem é a única opção — não existe /gas-tanks/:chainId).",
          "**2. Webhook (opcional)** — Cadastra um receptor para eventos em tempo real. Em produção, prefira webhook ao invés de polling.",
          "**3. Deposit** — Gera deposit address determinístico (CREATE2, captura `depositAddressId`), aguarda fundos, valida sweep para a hot wallet (com USD via CoinGecko), reconfere o forwarder via Multicall3.",
          "**4. Withdrawal** — Whitelist do destino (requer `X-2FA-Code`), criação do saque com idempotency key, self-approve em modo full-custody, espera por broadcast e confirmação on-chain.",
          "**5. Cleanup** — Remove o webhook de teste.",
          "**6. Activity & reporting** — Listagem enriquecida de forwarders (com 5 inputs do CREATE2 + rollup totalDeposits/lastDepositAt), depósitos filtrados por janela de data inclusiva, e o feed real de flush activity (sweeps + lazy deploys).",
          "**Self-service API key management** — JWT-only (endpoints rejeitam X-API-Key como bloqueio de privilege escalation). Lista/cria/revoga as próprias chaves usando o cookie de sessão do portal.",
        ],
      },
      {
        type: "heading",
      ...
      },
      {
        type: "callout",
        variant: "warning",
        title: "Whitelist exige 2FA (TOTP) no header",
        text: "POST /addresses requer o header X-2FA-Code com o código TOTP atual do app autenticador. Sem ele, a resposta é 401 (proteção contra exfiltração de API key). Preencha a variável `totp2faCode` no Postman antes de rodar o passo de whitelist; cada código vive ~30s.",
      },
      {
        type: "callout",
        variant: "info",
        title: "Endpoints JWT-only em Self-service API key management",
        text: "Os endpoints de listar/criar/revogar API keys (GET/POST/DELETE /api-keys) rejeitam X-API-Key com 401 — é um bloqueio explícito de privilege escalation. Use o JWT da sessão do portal: login em portal.vaulthub.live, DevTools → Application → Cookies → cvh_session, copie para a variável `portalJwt`.",
      },
      {
        type: "heading",
        level: 3,
      ...
      },
      {
        ...
        rows: [
          ...
          ["401", "Invalid or missing 2FA code", "Whitelist (POST /addresses) sem X-2FA-Code ou TOTP expirado", "Atualizar `totp2faCode` com código novo do autenticador e reenviar"],
          ["401", "API key not allowed for this endpoint", "Tentou usar X-API-Key em endpoint JWT-only (/api-keys CRUD)", "Trocar para o JWT de sessão (variável `portalJwt`)"],
          ["403", "Insufficient scope", "Key sem o escopo necessário (ex. wallets:read, withdrawals:hot)", "Gerar nova key com escopos corretos no Portal"],
          ["404", "Cannot GET /gas-tanks/56", "Endpoint per-chain não existe", "Use GET /gas-tanks e filtre por chainId no cliente"],
