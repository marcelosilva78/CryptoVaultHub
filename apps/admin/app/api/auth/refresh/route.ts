...
export async function POST(request: NextRequest) {
  ...
  if (!res.ok) {
    ...
    response.cookies.set('cvh_admin_token', '', { maxAge: 0, path: '/' });
    response.cookies.set('cvh_admin_refresh', '', { maxAge: 0, path: '/' });
    return response;
  }
  ...
  if (newRefresh) {
    response.cookies.set('cvh_admin_refresh', newRefresh, {
      httpOnly: true,
      secure: process.env.NODE_ENV === 'production',
      sameSite: 'lax',
      maxAge: 60 * 60 * 24 * 30,
